...
class ImageProcessingTransformer(nn.Module):
    ...
    def __init__(self, patch_size=1, in_channels=3, mid_channels=64, num_classes=1000, depth=12,
        ...
        self.task_embed = nn.Parameter(torch.zeros(6, 1, 1024, self.embed_dim))
...
def ipt_base(**kwargs): # 이거를 import해서 모델을 가져옴
    model = ImageProcessingTransformer(
        patch_size=4, depth=6, num_heads=4, ffn_ratio=4, qkv_bias=True,
